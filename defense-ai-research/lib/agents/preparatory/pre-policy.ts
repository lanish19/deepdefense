...
export class PrePolicyAgent extends BasePreparatoryAgent {
  async execute(): Promise<PreparatoryAgentOutput> {
    console.log('🏛️ Starting Pre-Policy Agent (G.1)');
...
YOUR TASK:  
Since you are conducting the first step in this national security ai-autonomy research process, your task is only aimed at helping prepare, plan, and tee-up the following deep research phase. To do so, please research online (use tools: "Search Web" and "Get Webpage") and come up with a plan for your policy researcher colleague performing the following intent of conducting specialized research analysis through a government and policy lens, focusing on what policymakers are saying, doing, implementing, and any other info regarding autonomous AI systems within his specific US national security domain: ${this.domainFocus}. 
...
4.) Identify 5 implicit questions that would generate highly insightful research outputs that are at a deep and expert (not simple and non-surface level). Provide these 5 in a list, along with short (20-40 word) description and justification for each why this question would be useful.`;
    ...
    const searchQueries = [
      `${this.domainFocus} AI policy government regulation`,
      `autonomous AI systems ${this.domainFocus} defense policy`,
      `AI governance ${this.domainFocus} national security strategy`,
      `policymakers artificial intelligence ${this.domainFocus} implementation`
    ];
    ...
    try {
      ...
      console.log('✅ Pre-Policy Agent completed');
      return result;
    } catch (error) {
      console.error('❌ Pre-Policy Agent failed:', error);
      throw error;
    }
  }
} 