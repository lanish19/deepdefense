...
export class PrePrimesAgent extends BasePreparatoryAgent {
  async execute(): Promise<PreparatoryAgentOutput> {
    console.log('🏢 Starting Pre-Primes Agent (G.2)');
...
YOUR TASK:  
Since you are conducting the first step in this national security ai-autonomy research process, your task is only aimed at helping prepare, plan, and tee-up the following deep research phase. To do so, please research online (use tools: "Search Web" and "Get Webpage") and come up with a plan for your market and firm researcher colleague performing the following intent of conducting specialized research analysis focused on major public defense and/or cyber firms within this domain (${this.domainFocus}) examining the go-to-market strategies, commentary, investments, new tech, and initiatives of these firms in the US and allied nations.
...
4.) Identify 5 implicit questions that would generate highly insightful research outputs that are at a deep and expert (not simple and non-surface level). Provide these 5 in a list, along with short (20-40 word) description and justification for each why this question would be useful.`;
    ...
    const searchQueries = [
      `major defense contractors ${this.domainFocus} AI investments`,
      `Lockheed Martin Raytheon Boeing ${this.domainFocus} artificial intelligence`,
      `defense primes autonomous systems ${this.domainFocus}`,
      `public defense companies AI initiatives ${this.domainFocus} market strategy`
    ];
    ...
    try {
      ...
      console.log('✅ Pre-Primes Agent completed');
      return result;
    } catch (error) {
      console.error('❌ Pre-Primes Agent failed:', error);
      throw error;
    }
  }
} 