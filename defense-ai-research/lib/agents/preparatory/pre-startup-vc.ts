...
export class PreStartupVCAgent extends BasePreparatoryAgent {
  async execute(): Promise<PreparatoryAgentOutput> {
    console.log('🚀 Starting Pre-Startup/VC Agent (G.3)');
...
YOUR TASK:  
Since you are conducting the first step in this national security ai-autonomy research process, your task is only aimed at helping prepare, plan, and tee-up the following deep research phase. To do so, please research online (use tools: "Search Web" and "Get Webpage") and come up with a plan for your startup/VC researcher colleague performing the following intent of conducting specialized research analysis focused on startups and VCs active in this domain (${this.domainFocus}) investigating what emerging startups and VCs are saying, doing, and investing within this domain examining the go-to-market strategies, commentary, investments, new tech, initiatives, ect
...
4.) Identify 5 implicit questions that would generate highly insightful research outputs that are at a deep and expert (not simple and non-surface level). Provide these 5 in a list, along with short (20-40 word) description and justification for each why this question would be useful.`;
    ...
    const searchQueries = [
      `${this.domainFocus} startups AI venture capital funding`,
      `emerging AI companies ${this.domainFocus} defense technology`,
      `VC investments autonomous systems ${this.domainFocus}`,
      `startup ecosystem ${this.domainFocus} artificial intelligence funding rounds`
    ];
    ...
    try {
      ...
      console.log('✅ Pre-Startup/VC Agent completed');
      return result;
    } catch (error) {
      console.error('❌ Pre-Startup/VC Agent failed:', error);
      throw error;
    }
  }
} 